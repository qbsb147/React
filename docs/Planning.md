...
데이터는 json-server를 활용해서 방문하고 분석 및 가공하여 시각적으로 표현한다.
...
- AGGrid와 달리 무료로 이용 가능하다
- ECharts와 달리 React 컴포넌트 기반이다, JSX로 조립하는 방식이다.
- ChartJs는 쉽게 시작할 수 있지만 지원하는 차트가 제한적이다.
- 공식 페이지의 문서화가 매우 잘 되어 있는 것을 확인할 수 있었다.
...
2.  대시 보드 Overview 📈  
    KPI 카드 :  
    ![img.png](./images/img.png) - 하루 사용자 수 - KPI 카드로 전날 대비 변화율을 나타낸다.
    ...
    차트 : - 이벤트 트렌드(멀티 라인 차트) - 전환 퍼널 - 상위 페이지 TOP5
...
| **상황**                | **처리 방식**                                   |
| ----------------------- | ----------------------------------------------- |
| API 타임아웃 (5초 초과) | 에러 상태 전환 + "재시도" 버튼 조회             |
| 필터 결과 0건           | "해당 조건의 데이터가 없습니다" 빈 상태 UI      |
| 날짜 범위 역전          | 선택 불가 혹은 선택 즉시 경고 토스트, 방문 차단 |
